...
function useOnScroll(elements, onScroll, environment, trackScroll) {
  ...
  const memoElements = React.useMemo(() => typeof environment !== "undefined" ? [environment, ...elements] : [], [elements]);
...
function useIsOpen(internal, external) {
  const shouldOpenAfterMount = React.useRef(external);
...
function ToggleLayer(_ref) {
  ...
  useOutsideClick(outsideClickRefs, React.useCallback(() => {
    if (!isOpen) {
      return;
    }
...
function useToggleLayer(renderLayer, _temp) {
  let _ref = _temp === void 0 ? {} : _temp,
    ...
    environment = typeof window !== "undefined" ? window : undefined
      ...
      props = objectWithoutPropertiesLoose(_ref, ["onStyle", "closeOnOutsideClick", "closeOnDisappear", "fixed", "container", "placement", "environment"]);
  ...
  const layerRef = React.useRef(null);
  const triggerRectRef = React.useRef(null);
  ...
  const element = relativeParentElement && reactDom.createPortal(renderLayer({
    layerProps: {
      ref: element => {
        if (element) {
          // observe the layer for resizing
...
function useTooltip(renderLayer, _temp) {
  let _ref = _temp === void 0 ? {} : _temp,
    ...
    hideOnScroll
      ...
      rest = objectWithoutPropertiesLoose(_ref, ["delayEnter", "delayLeave", "hideOnScroll"]);
