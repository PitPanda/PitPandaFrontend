...
class ItemSearch extends React.Component {
  ...
  async query(queryString) {
    ...
    const response = await axios
      .get(`/api/itemsearch/${queryString}`)
      .catch((r) => r);
  ...
  updatePath = (queryString) =>
    this.props.history.push(`/itemsearch/${queryString}`);
  ...
  loadMore = async () => {
    ...
    const response = await axios
      .get(`/api/itemSearch/${this.state.queryString}`, {
  ...
  render() {
    return (
      <div style={{ textAlign: "center", margin: "auto" }}>
        <h1 className="page-header" style={{ marginBottom: "200px" }}>
          Pit Panda Mystic Search
        </h1>
        ...
        >
          <StaticCard title="Results">
            ...
            !this.state.loading ? (
              ...
              >
                <button onClick={this.loadMore} className="srchBtn">
                  Load More
                </button>
              </div>
            ) : (
