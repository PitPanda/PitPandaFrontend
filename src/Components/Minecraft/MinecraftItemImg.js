...
function MinecraftItemImg(props) {
  ...
  const inner =
    count > 1 ? <span className="textshadow count">{count}</span> : "";
  let style = JSON.parse(JSON.stringify(props.style || {}));
  if (id >= 298 && id <= 301 && !(id === 300 && mapped.includes(meta))) {
    //leather items
    style.backgroundColor = `#${meta}`;
    return (
      <div
        id="item"
        className={`item_ item_${id}  ${count === 0 ? "grey" : ""}`}
        style={style}
        children={inner}
      />
    );
    ...
    return (
      <div
        id="item"
        className={`item_ item_${id} item_${id}_${meta} ${
          count === 0 ? "darken" : ""
        }`}
        style={style}
        children={inner}
