...
const formatted = Object.entries(Mystics)
  ...
  .sort(([, a], [, b]) =>
    a.Name.substring(a.Name.indexOf("§9") + 2) <
    b.Name.substring(b.Name.indexOf("§9") + 2)
      ? -1
      : 1
  );
...
class QueryBox extends React.Component {
  ...
  killInput = (index) => {
    if (
      (index !== this.state.inputs.length - 1 && index !== 0) ||
      (index !== 0 && this.state.inputs[index - 1].report === "")
    ) {
  ...
  buildAndSendQuery = () => {
    let inputs = this.state.inputs;
    let toClear = inputs.filter(
      (input, index) => input.reporting === "" && index !== inputs.length - 1
    );
  ...
  focus = (index) => {
    ...
    } else if (index >= this.state.inputs.length)
